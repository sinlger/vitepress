...
breadcrumb:
  ...
  - - link: /zh/Angle/
      linkText: 角度转换
  - - link: /zh/Angle/deg-to-arcsec
      linkText: 度数 (°) 到 角秒 (arcsec) 换算
head:
  - - meta
    - name: description
      content: "将度数(°)转换为角秒(arcsec)的在线工具和换算公式。提供简单易用的角度单位转换计算器。"
  - - meta
    - name: keywords
      content: "角度转换, 度数转角秒, °到arcsec换算, 在线计算器, 数学工具, 单位转换"
---
# 度数 (°) 到 角秒 (arcsec) 的换算
---
<script setup>
...
从 **度数 (°)** 转换到 **角秒 (arcsec)** 的公式为：
$$ arcsec = ° \times 3600 $$

## 度数到角秒换算指南

角度测量是数学和工程领域中的基础概念之一，尤其在几何学、物理学以及计算机图形学中应用广泛。本文将介绍如何将度数 (°) 转换为角秒 (arcsec)，并提供一些实用示例。
